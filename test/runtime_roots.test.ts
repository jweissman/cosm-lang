...
test("member access can inspect the class repository", () => {
  ...
  expect(cosmEval("classes.Object.methods.get(:method).name")).toBe("method");
  ...
  expect(cosmEval("classes.Function.methods.get(:call).name")).toBe("call");
  expect(cosmEval("classes.Method.methods.get(:call).name")).toBe("call");
  expect(cosmEval("classes.Symbol.methods.get(:eq).name")).toBe("eq");
  ...
  expect(cosmEval("classes.Kernel.methods.get(:assert).name")).toBe("assert");
  expect(cosmEval("classes.Process.methods.get(:cwd).name")).toBe("cwd");
  expect(cosmEval("classes.Process.methods.get(:argv).name")).toBe("argv");
  expect(cosmEval("classes.Process.methods.get(:pid).name")).toBe("pid");
  expect(cosmEval("classes.Process.methods.get(:exit).name")).toBe("exit");
  expect(cosmEval("classes.Time.methods.get(:now).name")).toBe("now");
  expect(cosmEval("classes.Time.methods.get(:isoNow).name")).toBe("isoNow");
  expect(cosmEval("classes.Time.methods.get(:iso).name")).toBe("iso");
  expect(cosmEval("classes.Random.methods.get(:float).name")).toBe("float");
  expect(cosmEval("classes.Random.methods.get(:int).name")).toBe("int");
  expect(cosmEval("classes.Process.methods.get(:env).name")).toBe("env");
  expect(cosmEval("classes.Http.methods.get(:serve).name")).toBe("serve");
  expect(cosmEval("classes.Http.methods.get(:request).name")).toBe("request");
  expect(cosmEval("classes.HttpRequest.methods.get(:bodyText).name")).toBe("bodyText");
  ...
  expect(cosmEval("classes.HttpServer.methods.get(:stop).name")).toBe("stop");
...
test("modules, views, and runtime roots expose predictable reflective surfaces", () => {
  ...
  expect(cosmEval('Cosm.version')).toBe("0.3.13.37");
...
test("Kernel, Process, Time, and Random expose tie-your-shoes runtime helpers", () => {
  expect(cosmEval('classes.Kernel.methods.get(:assert).call(true, "ok")')).toBe(true);
