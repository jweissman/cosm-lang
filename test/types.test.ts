...
test("core runtime manifests expose a consistent boot surface", () => {
  ...
  expect(Object.keys(mirrorClassMethods).sort()).toEqual(["reflect", "status"]);
  ...
  expect(Object.keys(aiMethods).sort()).toEqual(["cast", "chat_cast", "compare", "complete", "config", "health", "resolve", "status", "stream"]);
