...
In `0.3.2`, stabby lambdas remain the only standalone parameterized lambda form. Trailing call blocks may now bind parameters like `get "/" do |req| ... end`, but block capture and forwarding are still intentionally deferred.
...
Inside `class << self ... end`, `self` is the class object, and defs become class-side methods. In `0.3.2`, this is equivalent to existing `def self.name(...)` behavior rather than a second metaclass semantics.
...
That trailing block form is intentionally narrow in `0.3.2`: it is still just sugar for an extra final lambda argument. It now supports block parameters on trailing call blocks, but it does not yet support ampersand-style capture/forwarding.
...
Local `.cosm` files may also be loaded through `require("path/to/file.cosm")`. In `0.3.2`, this injects a basename-style module binding into the current scope, so `require("app/app.cosm")` makes `app` available as a reflective `Module` object.
...
- `router.use(middleware)`
  Tiny exact-path router helpers. In `0.3.2`, routes match on exact method + exact path only. Unmatched routes return a plain `404` response, invalid handler registration errors are raised immediately, and router-level middleware may wrap dispatch through `next()`.
- `HttpRequest.method`
- `HttpRequest.url`
- `HttpRequest.path`
- `HttpRequest.headers`
...
- `router.draw do ... end` and `get "/" do |req| ... end` are the intended `0.3.2` routing ergonomics boundary. They are still just final-argument block sugar over the existing callable model, not a full block system.
...
- Triple-quoted strings are the first simple multiline interpolated template form in `0.3.2`; raw/tagged heredoc variants are still deferred.
...
- `HttpRouter` is intentionally exact-path and object-first in `0.3.2`; route params, wildcards, middleware groups/macros, and richer route DSLs are still deferred.
- `Mirror` is intentionally readonly and observational in `0.3.2`; it is not yet a JS bridge, proxy, or hologram-style presenter.
