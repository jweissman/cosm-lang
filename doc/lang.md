...
- `~=` is the explicit semantic comparison seam in `0.3.13.42`. `as` is the structured semantic-cast surface, and `Cosm::AI.resolve(...)` is now the explicit semantic-decision seam for choosing among options. For string/symbol option sets it returns an enum-tag value, so predicate-style branching like `action.review?` stays aligned with the `as Model` story. `~` is still intentionally deferred.
