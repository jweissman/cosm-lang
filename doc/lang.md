...
- `~=` is the explicit semantic comparison seam in `0.3.13.41`. `as` is the structured semantic-cast surface, and `Cosm::AI.resolve(...)` is now the explicit semantic-decision seam for choosing among options. `~` is still intentionally deferred.
