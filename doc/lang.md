...
- `~=` is the explicit semantic comparison seam in `0.3.13.40`. `as` is now the taught structured semantic-cast surface, and it lowers to the same underlying `Cosm::AI.cast(...)` runtime seam. `~` is still intentionally deferred.
