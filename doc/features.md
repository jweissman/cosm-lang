...
## Current Release Target: 0.3.13.42

`0.3.13.24`-`0.3.13.42` are best read as the hardening, canonicalization, boundary-formalization, example-corpus/VM-corridor, ergonomics, host-boundary-proof, tooling-consolidation, core-programmability, intrinsic-ownership, reflection-cleanup, VM-legibility, thesis-alignment, and thesis-coherence line that follows the core-tower/authored-facade work:
...
- keep the explicit `BasicObject` / `Object` / `Module` / `Class` split legible through authored facades and smaller runtime seams
- keep `Collection`, `Enumerable`, `Sequence`, and `Mapping` as the explicit authored collection story
...
- classify the remaining TS-native runtime surfaces more explicitly as substrate, transitional wrappers, or authored-policy candidates
- keep pushing ordinary reflection/presence/ancestry helpers into `cosm/core/*`
...
- A first reflective `Module` runtime object, with maintained module surfaces such as `Cosm::Spec` modeled as real module objects rather than loose namespaces.
- Local `.cosm` files can now be loaded as reflective `Module` objects through `require("path/to/file.cosm")`, and `require(...)` may now be used as an expression so explicit bindings like `let slack = require("lib/agent/slack.cosm")` are the preferred style. In `0.3.13.12`, basename auto-injection remains only as a compatibility path for one patch line. `.ecosm` files also load through `require(...)` as renderable module objects, which gives the app layer a cleaner `app/views/...` template boundary without introducing a framework.
...
- `Cosm::Spec` is now the canonical Cosm-native test harness, layered on top of `Kernel.test(...)`, `Kernel.describe(...)`, and `Kernel.testSummary()`.
- `Kernel.sleep(ms)`, `Kernel.eval(source)`, `Kernel.tryEval(source)`, `Kernel.try(block)`, `Kernel.tryValidate(value, schemaOrModel)`, `Kernel.trace(...)`, `Kernel.readline(prompt?)`, `Kernel.raise(...)`, and `Kernel.resetSession()` now provide a tiny synchronous comfort layer for service/notebook work. In `0.3.12.x`, those eval helpers still delegate to `Session.default()` instead of a hidden shared env, and `Kernel.tryEval(...)` / `Kernel.tryValidate(...)` return structured namespaces whose `.error` is a first-class `Error` object.
...
- Live localhost round-trip checks for that HTTP surface now live in `test/http.integration.slow.ts` and run explicitly with `COSM_HTTP_INTEGRATION=1` rather than in the default fast suite.
...
- explicit module bindings are now the documented style for newer Cosm-authored code, even while compatibility auto-injection still exists
- the newer agent/support layer reads in idiomatic snake_case rather than TS-shaped helper naming
